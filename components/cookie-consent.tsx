...
export function CookieConsent({
  ...
  return variant === "default" ? (
    <div
      className={cn(
        "fixed z-200 bottom-0 right-0 right-0 p-4 sm:p-0 sm:right-4 sm:bottom-4 w-full sm:max-w-md duration-700",
        !isOpen
      ...
      <div className="dark:bg-card bg-background rounded-lg sm:rounded-md border border-border shadow-lg">
        <div className="grid gap-2">
          ...
          <div className="grid grid-cols-2 items-center gap-2 p-3 sm:p-4 sm:py-5 border-t border-border dark:bg-background/20">
            <Button onClick={accept} variant="default" className="w-full cursor-pointer">
              Accept
            </Button>
            <Button onClick={decline} variant="outline" className="w-full cursor-pointer">
              Decline
            </Button>
          </div>
        </div>
    ...
    variant === "minimal" && (
      <div
        className={cn(
          "fixed z-200 bottom-0 right-0 right-0 p-4 sm:p-0 sm:right-4 sm:bottom-4 w-full sm:max-w-[300px] duration-700",
          !isOpen
            ? "transition-[opacity,transform] translate-y-8 opacity-0"
            : "transition-[opacity,transform] translate-y-0 opacity-100",
          hide && "hidden"
